...
public class CustomerDao {
	...
	public List<User> getAllCustomer() {
		...
		try {
			 Connection con=DbConnection.getConnection();
			 String query="select uId,uName,uEmail,uPass,uAdd,uPhone,uImg,createdDate,updatedDate,status from user where roleId=2";
			 PreparedStatement  ps=con.prepareStatement(query);
	...
	public User getCustomerById(int uId) {
		...
		try {
			 Connection con=DbConnection.getConnection();
			 String query="select uId,uName,uEmail,uPass,uAdd,uPhone,uImg,createdDate,updatedDate,status from user where roleId=2 and uId=?";
			 PreparedStatement  ps=con.prepareStatement(query);
			 ps.setInt(1, uId);
			 ResultSet rs=ps.executeQuery();
			 while (rs.next()) {
	...
	public int UpdateCustomer(User u) {
		...
		try {
		   Connection con=DbConnection.getConnection();
		   String query="update user set uName=?,uEmail=?,uPass=?,uAdd=?,uPhone=?,uImg=?,updatedDate=now(),status=? where roleId=2 and uId=?";
		   PreparedStatement  ps=con.prepareStatement(query);
		   ps.setString(1, u.getUName());
		   ps.setString(2, u.getUEmail());
		   ps.setString(3, u.getUPass());
